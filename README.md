...
| `targetedRegionsAboveMinCoverageDeviation`   | Percent deviation of computed metric (`targetedRegionsAboveMinCoverage`) from provided metric (`targetedRegionsAboveMinCoverageProvided`).                                                                                                                                                                        |
| `targetedRegionsAboveMinCoverageQCStatus`    | <ul>  <li>`THRESHOLD NOT MET` if computed metric lower than required metric (`targetedRegionsAboveMinCoverage < targetedRegionsAboveMinCoverageRequired`); fails quality control</li>  <li>`TOO LOW` if deviation lower than -10% (`targetedRegionsAboveMinCoverageDeviation < -10`); reported, but does not fail quality control</li>  <li>`PASS`, otherwise</li></ul> |
...
Additionally, a computed metric that deviates by more than 10% below the value provided in the submission metadata is flagged as `TOO LOW` in the per-metric QC status columns. Such deviations point to a quality problem and are reported to the Plattformträger, but they do not by themselves fail quality control as long as the computed metrics still meet the required thresholds.
