...
| `qualityControlStatus`                       | Sample quality control status: `PASS` if every computed metric meets the corresponding metric required by BfArM, `FAIL` otherwise. Deviations from the provided metrics are reported per metric and do not affect this status.                                                                                     |
...
| `meanDepthOfCoverageDeviation`               | Signed percent deviation between computed (`meanDepthOfCoverage`) and provided (`meanDepthOfCoverageProvided`) coverage, relative to the computed value. Negative if the provided value is higher.                                                                                                                 |
| `meanDepthOfCoverageQCStatus`                | <ul>  <li>`THRESHOLD NOT MET` if computed metric lower than required metric (`meanDepthOfCoverage < meanDepthOfCoverageRequired`); fails quality control</li>  <li>`DEVIATION` if the absolute deviation exceeds 10% (`abs(meanDepthOfCoverageDeviation) > 10`); reported, but does not fail quality control</li>  <li>`PASS`, otherwise</li></ul> |
...
| `percentBasesAboveQualityThresholdDeviation` | Signed percent deviation between computed (`percentBasesAboveQualityThreshold`) and provided (`percentBasesAboveQualityThresholdProvided`) metric, relative to the computed value. Negative if the provided value is higher.                                                                                       |
| `percentBasesAboveQualityThresholdQCStatus`  | <ul>  <li>`THRESHOLD NOT MET` if computed metric lower than required metric (`percentBasesAboveQualityThreshold < percentBasesAboveQualityThresholdRequired`); fails quality control</li>  <li>`DEVIATION` if the absolute deviation exceeds 10% (`abs(percentBasesAboveQualityThresholdDeviation) > 10`); reported, but does not fail quality control</li>  <li>`PASS`, otherwise</li></ul> |
...
| `targetedRegionsAboveMinCoverageDeviation`   | Signed percent deviation between computed (`targetedRegionsAboveMinCoverage`) and provided (`targetedRegionsAboveMinCoverageProvided`) metric, relative to the computed value. Negative if the provided value is higher.                                                                                           |
| `targetedRegionsAboveMinCoverageQCStatus`    | <ul>  <li>`THRESHOLD NOT MET` if computed metric lower than required metric (`targetedRegionsAboveMinCoverage < targetedRegionsAboveMinCoverageRequired`); fails quality control</li>  <li>`DEVIATION` if the absolute deviation exceeds 10% (`abs(targetedRegionsAboveMinCoverageDeviation) > 10`); reported, but does not fail quality control</li>  <li>`PASS`, otherwise</li></ul> |
...
The BfArM criteria distinguish two comparisons, and this pipeline implements the second one:

- The _Grundprüfung_ compares the metrics **submitted** by the Leistungserbringer against the required thresholds. This is validated when the submission metadata is parsed, by `grz-pydantic-models`.
- The _Detailprüfung_ compares the metrics **computed** by this pipeline against the same required thresholds. A sample fails quality control only if a computed metric falls below its required threshold.

Additionally, whenever the value provided in the submission metadata deviates by more than 10% from the value computed by the pipeline (in either direction, relative to the computed value), the metric is flagged as `DEVIATION` in the per-metric QC status columns. Such deviations point to a quality problem and are reported to the Plattformträger, but they do not by themselves fail quality control as long as the computed metrics still meet the required thresholds.
