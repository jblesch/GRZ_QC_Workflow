...
Additionally, whenever the value provided in the submission metadata deviates by more than 10% from the value computed by the pipeline (in either direction, relative to the computed value), the metric is flagged as `DEV > 10%` in the per-metric QC status columns. Such deviations point to a quality problem and are reported to the Plattformträger, but they do not by themselves fail quality control as long as the computed metrics still meet the required thresholds.
